...
logger.info("Setting Printed Items to Printed Status IP");
      ...
      PrintedService.iprintSetToPrinted(data).then(response => {
        ...
        if (i == (itemIds.length - 1)) {
          ...
          fsExtra.move(filePath, newFilePath, (err) => {
            ...
            logger.info("Moved Barcode Files IP");
          });
          ...
          fsExtra.move(artworkPath, newArtworkPath, (err) => {
            ...
            logger.info("Moved Artwork Files IP");
          })
          console.log("Moved Files IP");
          logger.info("Moved Files IP");
        }
      }).catch(err => {
        console.log(err);
        logger.error(err);
